...
const history = [
  { year: '2018', title: 'The Beginning', desc: 'Started in a small garage in Eindhoven with a passion for bikes.' },
  { year: '2020', title: 'First Store', desc: 'Opened our flagship store and hired the first mechanics.' },
  { year: '2022', title: 'Going Digital', desc: 'Launched our e-commerce platform to serve the entire region.' },
  { year: '2024', title: 'Expansion', desc: 'Moved to a larger HQ to accommodate our growing team and inventory.' },
  { year: '2025', title: 'Future', desc: 'Developing our own custom line of electric off-road bikes.' }
];
</script>
  ...
  <div class="flex justify-center w-full min-h-screen pb-10">
    ...
    <div class="flex flex-col w-full max-w-[1800px] mx-3 lg:mx-5 mt-5">
      ...
      <PageHero
          ...
          :tagline="$t('company.about.tagline')"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 flex flex-col gap-8">
          ...
          <div class="bg-white dark:bg-black border border-gray-100 dark:border-neutral-800 rounded-[32px] p-8 lg:p-10 shadow-sm">
            <h3 class="text-xl font-bold mb-6 text-black dark:text-white">
              {{ $t('company.about.video.title') || 'See us in action' }}
            </h3>
            <div class="relative w-full rounded-2xl overflow-hidden aspect-video bg-neutral-100 dark:bg-neutral-900 mb-6">
              <video
                  controls
                  class="w-full h-full object-cover"
                  poster="https://images.unsplash.com/photo-1665045622636-ee4ce6e362ac?q=80&w=1740&auto=format&fit=crop"
              >
            ...
            <p class="text-neutral-600 dark:text-neutral-400 leading-relaxed text-lg">
              {{ $t('company.about.video.desc', { company: siteName }) }}
            </p>
          </div>
              ...
              <div v-for="member in teamMembers" :key="member.id" class="group">
                <div class="aspect-square rounded-2xl overflow-hidden bg-gray-200 dark:bg-neutral-800 mb-4 relative">
                  <NuxtImg
                      ...
                      :alt="member.name"
                      loading="lazy"
                      class="w-full h-full object-cover transition-all duration-500 grayscale group-hover:grayscale-0 group-hover:scale-110 opacity-0"
                      @load="$event.target.classList.remove('opacity-0')"
                  />
                </div>
                <h4 class="font-bold text-lg text-black dark:text-white">{{ member.name }}</h4>
                <p class="text-sm text-brand dark:text-brand-d font-medium">{{ member.role }}</p>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
          ...
          <div class="bg-white dark:bg-black border border-gray-100 dark:border-neutral-800 rounded-[32px] p-8 shadow-sm">
            ...
            <div class="flex flex-col gap-8">
              <div v-for="(val, idx) in values" :key="idx" class="flex gap-4">
                <div class="shrink-0 w-12 h-12 rounded-full bg-alizarin-crimson-50 dark:bg-brand/30 flex items-center justify-center text-brand dark:text-brand-d">
          ...
          <div class="bg-white dark:bg-black border border-gray-100 dark:border-neutral-800 rounded-[32px] p-8 shadow-sm flex-1">
            <h3 class="text-xl font-bold mb-8 text-black dark:text-white">Our Journey</h3>

            <div class="relative pl-2">
              <div v-for="(item, index) in history" :key="index" class="relative pl-8 pb-8 border-l border-gray-200 dark:border-neutral-800 last:pb-0 last:border-0">

                <div class="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-brand ring-4 ring-white dark:ring-black"></div>

                <div class="-mt-1.5">
                  <span class="text-xs font-bold text-brand uppercase tracking-wider mb-1 block">{{ item.year }}</span>
                  <h4 class="font-bold text-black dark:text-white mb-1">{{ item.title }}</h4>
                  <p class="text-sm text-neutral-500 dark:text-neutral-400 leading-snug">
                    {{ item.desc }}
                  </p>
                </div>
              </div>
          ...
          <div class="bg-black dark:bg-white rounded-[32px] p-8 text-white dark:text-black relative overflow-hidden group">
            ...
            <div class="absolute -bottom-10 -right-10 w-40 h-40 bg-brand dark:bg-brand-d rounded-full group-hover:scale-150 transition-transform duration-700 ease-in-out"></div>
          </div>
