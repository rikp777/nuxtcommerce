...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div v-if="pending" class="p-12 text-center text-neutral-500">
      <UIcon name="i-svg-spinners-ring-resize" class="text-2xl" />
    </div>
    ...
    <div v-else-if="error || !order" class="p-8 bg-red-50 dark:bg-red-900/10 text-red-600 rounded-[32px] border border-red-100 dark:border-red-900/30">
      <h3 class="font-bold mb-2">Order niet gevonden</h3>
      <p class="text-sm">Er is een fout opgetreden bij het ophalen van de order details. Mogelijk is de ID onjuist of heb je geen rechten.</p>
    ...
    <div v-else class="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-[32px] p-8 shadow-sm">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 border-b border-gray-100 dark:border-neutral-800">
        ...
        <div class="text-right">
          <p class="text-sm text-neutral-500">Totaalbedrag</p>
          <p class="text-2xl font-bold text-black dark:text-white">{{ order.total }}</p>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 py-8 border-b border-gray-100 dark:border-neutral-800">
        <div>
          <h3 class="font-bold text-black dark:text-white mb-3 uppercase text-xs tracking-wider">Factuuradres</h3>
        ...
        <div>
          <h3 class="font-bold text-black dark:text-white mb-3 uppercase text-xs tracking-wider">Verzendadres</h3>
          ...
          <p v-else class="text-sm text-neutral-400 italic">Geen verzendadres beschikbaar</p>
        </div>
      </div>

      <div class="py-8">
        <h3 class="font-bold text-black dark:text-white mb-4 uppercase text-xs tracking-wider">Artikelen</h3>
        <ul class="divide-y divide-gray-100 dark:divide-neutral-800">
          <li v-for="(item, i) in order.lineItems?.nodes" :key="i" class="py-4 flex items-center justify-between">
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="font-bold text-black dark:text-white text-sm">{{ item.product?.node?.name || 'Product niet meer beschikbaar' }}</p>
                <p class="text-xs text-neutral-500">Aantal: {{ item.quantity }}</p>
              </div>
            </div>
            <span class="font-bold text-black dark:text-white text-sm">{{ item.total }}</span>
          </li>
        </ul>
      </div>
      ...
      <div class="bg-gray-50 dark:bg-neutral-800/50 rounded-2xl p-6 space-y-2">
        ...
        <div class="pt-2 text-xs text-neutral-400 text-right">
          Betaald met {{ order.paymentMethodTitle }}
        </div>
      </div>
